...
| next decision | Review the implemented batch02 reference/timing diagnostic | The explicit v0.19.1 mode adds static failure details and numeric delta summaries while requiring all original counts to match the completed pilot. Ready for focused implementation/scope review and CI, then one private run; no new real result yet. EDP/RLMB is prioritized; direct LTSB work is on hold. The 376 sensor candidates are not eligible frames or residuals. See `docs/recording_pair_diagnostics_predeclaration.md`; frozen cohort gates remain unchanged |
