...
| v0.19.0 | Recording-level EDP/RLMB geometry pilot | Complete; PR #22 merged. All four files inspected with disk-backed geometry; 7,743 anchored candidates, 376 with SENSOR_TOPOLOGY, no residuals or outing admission |
| v0.19.1 | Reference-failure and numeric pair-timing diagnostic | Implementation/scope GO, zero blockers, passing Python 3.10/3.12 CI, and complete real output reconciled against every preserved count. All 29,569 generic reference errors are empty lane lists; all 7,743 anchored pairs have zero numeric source-time delta. PR #23 merged at `49ca002`; see `docs/recording_pair_diagnostics_batch02_result.md` |
| v0.19.2 | Exploratory batch02 residual/condition/sequence extraction | Implemented for focused review and CI before one private run. Preserves geometry, full old count/diagnostic parity and fixed features; explicitly checks odometry source/log chronology. Exports geometric profiles separately from complete-feature profiles, with file-local sequence support. No real extraction result or model yet; see `docs/exploratory_residuals_predeclaration.md` |
| next decision | Reconcile extraction, then declare an exploratory training protocol | Inspect geometric/complete-feature counts and actual contiguous transitions before choosing a minimal baseline. Unknown session provenance remains unknown; no final-cohort admission, automatic fit or LANE_MAP pooling. Direct LTSB stays on hold |
