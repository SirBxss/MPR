...
| v0.19.2 | Exploratory batch02 residual/condition/sequence extraction | Reviewed GO, Python 3.10/3.12 CI passed and one real output reconciled: 376 finite SENSOR EDP/RLMB pseudo-residuals, 134 complete conditions, 26 conditional sequences/108 transitions, longest 10 frames. No outing roles or fit; see `docs/exploratory_residuals_batch02_v0192_result.md` |
| next decision | Close PR #24; prioritize prospectively identified outings | Documentation-only closure/CI then merge. Do not fit AR/AIOHMM on short 134-row condition subset. A separately reviewed descriptive Gaussian protocol must validate this archive and compare families on identical complete rows with explicit development-only limits; independent-data admission remains blocked. Direct LTSB stays on hold |
