...
| v0.19.1 | Reference-failure and numeric pair-timing diagnostic | Implementation/scope GO, zero blockers, passing Python 3.10/3.12 CI, and complete real output reconciled against every preserved count. All 29,569 generic reference errors are empty lane lists; all 7,743 anchored pairs have zero numeric source-time delta. PR #23 open; see `docs/recording_pair_diagnostics_batch02_result.md` |
| next decision | Prepare bounded exploratory residual/condition/sequence extraction | Check the existing six causal inputs and recording-local contiguous support before exporting 21-station vectors. At most 376 sensor candidates under unchanged rules; no LANE_MAP pooling, invented session provenance, automatic fit or final-cohort admission. The new scope/code needs its own review before private execution; direct LTSB remains on hold |
